    ...
    def insert_numbers(self):
        for i in range(len(self.x)):
            for j in range(len(self.x[i])):
                self.screen.blit(self.font.render(str(self.x[i][j]), True, (0,0,0)), (20 * (i+self.x_additional) + 7, 20*j + 4))

        for i in range(len(self.y)):
            for j in range(len(self.y[i])):
                self.screen.blit(self.font.render(str(self.y[i][j]), True, (0,0,0)), (20*j + 7, 20 * (i+self.y_additional) + 4))
